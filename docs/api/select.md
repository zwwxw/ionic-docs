...
The `action-sheet` and `popover` interfaces do not have an `OK` button; clicking on any of the options will automatically close the overlay and select that value. The `popover` interface does not have a `Cancel` button either; clicking on the backdrop will close the overlay.
